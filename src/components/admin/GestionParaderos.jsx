...
import { PlusCircle, Edit3, Trash2, Plus, X, MapPin, Image } from 'lucide-react'; // Icons
...
const GestionParaderos = () => {
  ...
  const [longitud, setLongitud] = useState("");
  const [descripcion, setDescripcion] = useState("");
  const [imagen, setImagen] = useState(""); // Nuevo campo para URL de imagen
  const [paraderoEditando, setParaderoEditando] = useState(null);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // Añadir imagen solo si se proporciona una URL
      if (imagen.trim() !== "") {
        paraderoData.imagen = imagen;
      }
          ...
          <div className="mt-8 border-t pt-6 border-gray-200 dark:border-gray-600">
            ...
            <div className="bg-gray-100 dark:bg-gray-600 p-4 rounded-lg mb-4">
              ...
              <div className="flex justify-end">
                ...
                >
                  <Plus size={16} className="mr-1" /> Añadir Sitio
                </button>
              </div>
            </div>
